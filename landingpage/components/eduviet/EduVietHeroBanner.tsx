...
import { ArrowRight, Sparkles, Star, Award, BookOpen, Heart } from 'lucide-react';

interface EduVietHeroBannerProps {
  onActionClick?: () => void;
}

export default function EduVietHeroBanner({ onActionClick }: EduVietHeroBannerProps) {
      ...
      <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="space-y-2.5 max-w-xl">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-rose-100 text-rose-700 border border-rose-200 flex items-center gap-1">
              <Star className="w-3 h-3 text-amber-500 fill-amber-500" />
              <span>Giáo Dục Việt Nam</span>
            </span>
        ...
        <div className="hidden md:flex flex-col items-center justify-center p-3 rounded-2xl bg-white/90 dark:bg-slate-800/90 border border-amber-200 dark:border-slate-700 shadow-sm text-center max-w-[210px]">
          ...
          <p className="text-[10px] font-black text-rose-700 uppercase tracking-tight leading-snug">
            {slide.badge}
          </p>
          <div className="mt-1 flex items-center gap-1 text-[10px] font-semibold text-emerald-600">
            <Heart className="w-3 h-3 fill-rose-500 text-rose-500" />
            <span>Vì học sinh thân yêu</span>
          </div>
        </div>
      </div>
